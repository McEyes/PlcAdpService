...
{
    public class Fx3BinaryClient : PlcClient
    {
        public Fx3BinaryClient(IEquipmentConfig config, MqttClientService mqttClient, ILogger log = null) : base(config, mqttClient, log)
        {
...
//#if DEBUG2
//           Log.LogDebug($"[{NetConfig.Name}][{IP}]：exc {MCCmd} Hex:{ByteToHexString(cmd)} \r\n 0x:{BitConverter.ToString(cmd).Replace("-", " ")}");
//#endif
            return cmd;
        }
        ...
        {
#if DEBUG2
            Log.LogDebug($"[{NetConfig.Name}][{IP}]：exc {((Fx3MCCommandType)cmd[0])} Hex:{ByteToHexString(cmd)} \r\n 0x:{BitConverter.ToString(cmd).Replace("-", " ")}\r\n result Hex:{ByteToHexString(result)} \r\n 0x:{BitConverter.ToString(result).Replace("-", " ")}");
#endif
            if (result.Length < 2)
            {
                Log.LogError($"[{NetConfig.Name}][{IP}][{address}] error :{ByteToHexString(result)}");
                //throw new ArgumentException("error:" + ByteToHexString(result));
            }

            if (result[0] != cmd[0] + 0x80 || result[1] != 0x00)
            {
                Log.LogError($"[{NetConfig.Name}][{IP}][{address}] return 0, communication error :{ByteToHexString(result)}");
            ...
            {
                Log.LogError($"[{NetConfig.Name}][{IP}][{address}]return 0 or 1 or 15 or 16, communication error :{ByteToHexString(result)}");
                //throw new ArgumentException($"[{NetConfig.Name}][{IP}]：communication error:" + ByteToHexString(result));
            }
        }
        ...
        private byte[] WriteRaw(Fx3MCCommandType type, string address, byte length, params byte[] value)
        {
            ...
            {
                Log.LogWarning($"[{NetConfig.Name}][{IP}]：debug WriteRaw {type}[{address}] Hex:{ByteToHexString(cmd)} \r\n 0x:{BitConverter.ToString(cmd).Replace("-", " ")}");
            ...
            if (result.Success)
            {
                ...
                if(data.Length == 0)
                {
                    Log.LogWarning($"[{NetConfig.Name}][{IP}]：WriteRaw send {type}[{address}] 写入成功返回结果长度为0");
                    return new byte[] { 0x81, 0x00, 0x00, 0x90 };
                }
                //#if DEBUG2
                //                Log.LogDebug($"[{NetConfig.Name}][{IP}]：exc {type}[{address}] Hex:{ByteToHexString(cmd)} \r\n 0x:{BitConverter.ToString(cmd).Replace("-", " ")}\r\n result Hex:{ByteToHexString(data)} \r\n 0x:{BitConverter.ToString(data).Replace("-", " ")}");
                //#endif
                checkReadResult(address, cmd, data);
                return data;
            }
            return new byte[] { 0x81, 0x00, 0x00, 0x90 };
        }
        ...
        public override ushort[] ReadUInt16(string address, byte length)
        {
            ...
            var data = ReadRaw(Fx3MCCommandType.ReadByByte, address, length);
            if (data.Success)
            {
                for (int i = 0; i < length && data.Data.Length > 2 + i * 2; i++)
                {
                    result[i] = BitConverter.ToUInt16(data.Data, 2 + i * 2);
                }
            }
