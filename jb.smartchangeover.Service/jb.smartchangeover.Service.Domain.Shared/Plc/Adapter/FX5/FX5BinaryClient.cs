...
{
    public class Fx5BinaryClient : PlcClient
    {
        public Fx5BinaryClient(IEquipmentConfig config, MqttClientService mqttClient, ILogger log = null) : base(config, mqttClient, log)
        {
...
//#endif
            if (result.Length < 2)
            {
                Log.LogError($"[{NetConfig.Name}][{IP}][{address}] return length<2， error :{ByteToHexString(result)}");
            ...
            {
                Log.LogError($"[{NetConfig.Name}][{IP}][{address}] return 0 or 1 or 15 16 and length<3， communication error :{ByteToHexString(result)}");
                //throw new ArgumentException($"[{NetConfig.Name}][{IP}]：communication error:" + ByteToHexString(result));
            }
        }
        /// <summary>
        ...
        private byte[] WriteRaw(string address, byte length, byte[] cmdType, params byte[] value)
        {
            ...
            {
                Log.LogDebug($"[{NetConfig.Name}][{IP}]：debug WriteRaw [{address}] Hex:{ByteToHexString(cmd)} \r\n 0x:{BitConverter.ToString(cmd).Replace("-", " ")}");
        ...
        public override bool WriteUInt16(string address, params UInt16[] value)
        {
            ...
            byte[] result = WriteRaw(address, (byte)value.Length, Fx5UmcCommandType.WriteByWord, buffer);
            var check = result[result.Length - 1] == 0x00;
            if (!check)
            {
                Log.LogError($"[{NetConfig.Name}][{IP}] WriteRaw [{address}] Hex:{ByteToHexString(buffer)},result:{ByteToHexString(result)} ");
            }
            return check;
        }
        ...
        {
            byte[] result = WriteRaw(address, 1, Fx5UmcCommandType.WriteByBit, value ? (byte)0x10 : (byte)0x00);
