...
namespace jb.smartchangeover.Service.Domain.Shared.Plc.Adapter.Modbus
{
    public static class ModbusHelper
    {
        ...
        /// <param name="start"></param>
        /// <returns></returns>
        public static ushort[] ToUshorts(this string value, int start = 0)
        {
            byte[] bytesTemp = Encoding.UTF8.GetBytes(value);
            ushort[] dest = Bytes2Ushorts(bytesTemp);
            if (start > 0)
            {
                ushort[] src = new ushort[dest.Length - start];
                dest.CopyTo(src, start);
                return src;
            }
            else
        ...
        /// <param name="len"></param>
        /// <returns></returns>
        public static string ToString(this ushort[] src, int start=0, int len=0)
        {
            if (len == 0) { len = src.Length - start; }
            ushort[] temp = new ushort[len];
            for (int i = 0; i < len; i++)
            {
                temp[i] = src[i + start];
       ...
       /// <returns></returns>
        ...
        /// <returns></returns>
        public static bool CheckData(ushort[] src, ushort[] des)
        {
            if (des.Length < src.Length) return false;
